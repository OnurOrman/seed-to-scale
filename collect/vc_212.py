...
def collect_portfolio_links(url: str = "https://212.vc/portfolio") -> str:
  ...
  link_target_divs = [
    ...
    if "media-box" in " ".join(div.get("class", [])) and "category_" in " ".join(div.get("class", []))
  ]
  ...
  name_target_divs = [
    div
    for div in soup.find_all("div", class_=True)
    if "post_title-1" in " ".join(div.get("class", [])) and "mb_hide_if_empty mb_global_skin" in " ".join(div.get("class", []))
  ]
  ...
  portfolio_csv = csv_path / "212_portfolio_links.csv"

  if portfolio_csv.exists():
    existing_portfolio_df = pd.read_csv(portfolio_csv)
    combined_df = pd.concat([existing_portfolio_df, portfolio_df]).drop_duplicates(subset=["company_vc_link"], keep="last")
    combined_df.to_csv(portfolio_csv, index=False, encoding="utf-8")
  else:
    portfolio_df.to_csv(portfolio_csv, index=False, encoding="utf-8")

  return CSV_PATH + "/212_portfolio_links.csv"

def scrape_portfolio_links(path_to_links: str):
...
def collect_team_links(url: str = "https://212.vc/team") -> str:
  ...
  hidden_divs = [
    ...
    if all(cls in div.get("class", []) for cls in ["elementor-hidden-desktop", "elementor-hidden-tablet", "elementor-hidden-mobile"])
  ]
